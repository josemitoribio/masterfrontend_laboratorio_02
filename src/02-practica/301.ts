console.log("************** PRACTICE 301 - CALIFICATIONS SUMMARY *********************");
...
const studentsCalifications: Student[]= [
...
const summarizeClassRoom = (studentList: Student[]) => {
    const calculateCalifications = studentList.map(student => ({
        name: student.name,
        highestCalification: Math.max(...student.califications),
        averageCalifications: Number(student.califications.reduce((total, calification) => total + calification, 0) / student.califications.length)
        .toPrecision(3)
      }));

      return calculateCalifications;
};


console.log(summarizeClassRoom(studentsCalifications));
// [
